    ...
    def _on_packet_received(self, packet, interface=None):
        try:
            ...
            if portnum == 'ROUTING_APP':
                ...
                if rx_time:
                    try:
                        _rout_updates['last_heard'] = datetime.fromtimestamp(rx_time)
                    except Exception:
                        pass
                if from_id_string:
            ...
            if portnum == 'TRACEROUTE_APP':
                ...
                if rx_time:
                    try:
                        _tr_updates['last_heard'] = datetime.fromtimestamp(rx_time)
                    except Exception:
                        pass
                if from_id_string:
...
class MetricsTab(QWidget):
    ...
    def ingest_packet(self, packet: dict, node_data: dict):
        ...
        if portnum == 'TELEMETRY_APP':
            ...
            if dm:
                ...
                if ch  is not None and nid:
                    self._ch_util[nid] = float(ch)
                if air is not None and nid:
                    self._air_tx[nid]  = float(air)
                if batt is not None and nid:
                    self._battery[nid] = int(batt)
                # Série temporal: média de todos os nós
                if self._ch_util:
                    avg = sum(self._ch_util.values()) / len(self._ch_util)
                    self._ch_util_ts.append((ts, round(avg, 1)))
                    if len(self._ch_util_ts) > 30:
                        self._ch_util_ts = self._ch_util_ts[-30:]

        # Bateria de qualquer update
        batt2 = node_data.get('battery_level')
        if batt2 is not None and nid:
            self._battery[nid] = int(batt2)
    ...
    def ingest_message_status(self, req_id: int, status: str):
        ...
        Filtra pelo req_id: só conta se foi o nosso nó que enviou o pacote.
        Sem esta filtragem, ACKs de mensagens de outros nós inflacionam os contadores.
          'ack'          — destinatário confirmou recepção (entrega real)
          'ack_implicit' — relay local confirmou retransmissão (NÃO é entrega)
          'nak'          — falha definitiva com errorReason
        """
        if req_id not in self._sent_packet_ids:
            return   # ACK/NAK de mensagem de outro nó — ignorar
        if status == 'nak':
            self._msgs_naked += 1
            ...
            # Mantém o ID — pode ainda receber ACK real do destinatário
...
{body}
    ...
    def _data_nodes(self) -> dict:
        ...
        batt_avg=round(sum(self._battery.values())/len(self._battery),0) if self._battery else None
        batt_rows=sorted(self._battery.items(),key=lambda x:x[1])
        return {"n_active":n_active,"n_battery":len(self._battery),"batt_avg":batt_avg,
    ...
    def _data_reliability(self) -> dict:
        ...
        pending    = max(self._msgs_sent - total_resp - self._msgs_ack_implicit, 0)
...
window._metricsUpdateData = function(d) {{
  ...
  if(window._ppmChart) {{
    ...
    def _html_nodes(self) -> str:
        ...
        active_nids = set(p[1] for p in self._packets if p[0] >= cutoff2h and p[1])
        n_active = len(active_nids)
        n_battery = len(self._battery)

        # Série temporal de nós activos
        ts_labels = [self._ts_label(t) for t, _ in self._nodes_active_ts]
        ts_vals   = [v for _, v in self._nodes_active_ts]

        # Distribuição de bateria em buckets 20%
        batt_buckets = ['0-20%', '21-40%', '41-60%', '61-80%', '81-100%']
        batt_counts  = [0, 0, 0, 0, 0]
        for v in self._battery.values():
            idx = min(int(v // 20), 4)
            batt_counts[idx] += 1

        # Tabela de nós com bateria
        batt_rows = ""
        for nid, batt in sorted(self._battery.items(), key=lambda x: x[1]):
            color = "green" if batt > 60 else ("orange" if batt > 20 else "red")
            bar_c = "#39d353" if batt > 60 else ("#f0883e" if batt > 20 else "#f85149")
            batt_rows += (f"<tr><td>{self._name(nid)}</td>"
                          f"<td><span class='tag tag-{color}'>{batt}%</span></td>"
                          f"<td><div class='bar-bg'><div class='bar-fill' "
                          f"style='width:{batt}%;background:{bar_c}'></div></div></td></tr>")
        if not batt_rows:
            batt_rows = "<tr><td colspan='3' class='no-data'>Sem dados de bateria</td></tr>"

        body = f"""
<div class="subtitle">Saúde dos nós e baterias na rede</div>
<div class="grid-3">
  <div class="card"><h3>Nós Activos (2h)</h3><div class="kpi green">{n_active}</div></div>
  <div class="card"><h3>Com dados de bateria</h3><div class="kpi blue">{n_battery}</div></div>
  <div class="card"><h3>Bateria Média</h3>
    <div class="kpi {'green' if self._battery and sum(self._battery.values())/len(self._battery)>60 else 'orange'}">
    {round(sum(self._battery.values())/len(self._battery), 0) if self._battery else '—'}{'%' if self._battery else ''}
    </div>
  </div>
</div>
<div class="grid">
...
<div class="card" style="margin-top:16px">
  ...
  <table><tr><th>Nó</th><th>Nível</th><th>Barra</th></tr>{batt_rows}</table>
</div>
<script>
...
window._battDistChart = new Chart(document.getElementById('battDistChart'), {{
  type: 'bar',
  data: {{
    labels: {json.dumps(batt_buckets)},
...
window._metricsUpdateData = function(d) {{
  ...
  if(window._nodesChart && d.ts_vals.length > 0) {{
...
class MainWindow(QMainWindow):
  ...
  <line x1="67" y1="22" x2="88" y2="78" stroke="#67EA94" stroke-width="13" stroke-linecap="round"/>
    ...
    def _show_about_dialog(self):
        ...
        lbl_version = QLabel("Versão Gold  ·  2025")
